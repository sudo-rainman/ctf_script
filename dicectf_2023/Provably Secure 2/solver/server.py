...
""")
    for experiment in range(1, 2):
        ...
        key0 = rsa.generate_private_key(public_exponent=65537, key_size=2048,backend=default_backend())
        ...
        while True:
            ...
            if choice == 0:
                ...
                m0 = urandom(16)
                print("m0 ",m0)
                #bytes.fromhex(input("m0 (16 byte hexstring): ").strip())
                m1 = urandom(16) 
                print("m1 ",m1)
                #bytes.fromhex(input("m1 (16 byte hexstring): ").strip())
                ...
                if (de_count > 8):
                    ...
                    exit(0)
                in_ct = bytes.fromhex(seen_ct[0])[:256]
                in_ct1 = bytes.fromhex(seen_ct[0])[256:]
                print(bytes.fromhex(seen_ct[0]))
                print(len(bytes.fromhex(seen_ct[0])))
                in_ct2 = in_ct1 + in_ct
            
                    #input("ct (512 byte hexstring): ").strip())
                if len(in_ct2) != 512:
                    print("Must be 512 bytes!")
                    exit(0)
                if in_ct2.hex() in seen_ct:
                    print("Cannot query decryption on seen ciphertext!")
                    exit(0)
                print(decrypt(key0, key1, in_ct2).hex())
